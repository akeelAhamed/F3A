 ...
 * @link    http://www.vancelucas.com/ | https://github.com/vlucas/valitron
 */
class Validator
{
    /**
    ...
    public static function langDir($dir = null)
    {
        ...
        return static::$_langDir ?: dirname(dirname(__DIR__)) . '/Libraries/lang';
    }
    ...
    /**
     * Validates whether it is file and has mime type.
     *
     * @param string $field name of the field in the data array
     * @param mixed $value value of this field
     * @param array $params parameters for this rule
     * @return bool
     */
    protected function validateMime($field, $value, $params)
    {
        $mimeType = array(
            'txt' => 'text/plain',
            'htm' => 'text/html',
        ...
        if(isset($value['type'])){
            if(is_array($value['type'])){
                for ($i=0; $i < count($value['type']); $i++) { 
                    $type = $value['type'][$i];
                    $allow = array_intersect_key($mimeType, array_flip($params));
    ...
    /**
     * Validates whether it is file and size in kb.
     *
     * @param string $field name of the field in the data array
     * @param mixed $value value of this field
     * @param array $params parameters for this rule
    ...
    protected function validateSize($field, $value, $params)
    {
        ...
        $params = $params[0];
        $size = $params*$byte;
        if(isset($value['size']) && is_numeric($params)){
            if(is_array($value['type'])){
                for ($i=0; $i < count($value['size']); $i++) {
                    if ($value['size'][$i] > $size) {
                        return false;
                    }
                }
                return true;
